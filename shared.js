...
// Système de thèmes CSS variables — 5 thèmes
// ═══════════════════════════════════════════════════════════

// ── Gestion des thèmes ─────────────────────────────────────
(function ThemeSystem(){
  const THEMES = [
    { id:'1', name:'Institutionnel', icon:'🏛️', swatch:'linear-gradient(135deg,#1e3a8a,#197d89)' },
    { id:'2', name:'Dark Teal',      icon:'🌊', swatch:'linear-gradient(135deg,#0d1117,#0891b2)' },
    { id:'3', name:'Sable Chaud',    icon:'🏜️', swatch:'linear-gradient(135deg,#2d2416,#c2410c)' },
    { id:'4', name:'Forêt',          icon:'🌿', swatch:'linear-gradient(135deg,#14532d,#22c55e)' },
    { id:'5', name:'Violet Dusk',    icon:'🌌', swatch:'linear-gradient(135deg,#13111f,#7c3aed)' },
  ];
  window.THEMES = THEMES;

  function applyTheme(id){
    document.documentElement.setAttribute('data-theme', String(id));
    localStorage.setItem('cd47_theme', String(id));
  }
  function getCurrentTheme(){
    return localStorage.getItem('cd47_theme') || null;
  }
  window.applyTheme = applyTheme;
  window.getCurrentTheme = getCurrentTheme;

  // Applique immédiatement si déjà choisi
  const saved = getCurrentTheme();
  if(saved) applyTheme(saved);
})();

// ── CSS de base injecté (reset + styles non thémables) ─────
(function injectCSS(){
  const s = document.createElement('style');
  s.textContent = `
/* Override sidebar-year option background selon thème */
[data-theme="2"] .sidebar-year option,
[data-theme="5"] .sidebar-year option { background:#1e293b; }
[data-theme="3"] .sidebar-year option { background:#2d2416; }
[data-theme="4"] .sidebar-year option { background:#14532d; }

/* Override cal-year-sel */
[data-theme="2"] .cal-year-sel option { background:#0d1117; }
[data-theme="5"] .cal-year-sel option { background:#13111f; }

/* Sidebar background via inline var() — surcharge le style inline */
.sidebar { background: var(--sidebar-bg) !important; }

/* Spinner thème sombre */
[data-theme="2"] .spinner,
[data-theme="5"] .spinner { border-color:rgba(255,255,255,.15); border-top-color:rgba(255,255,255,.8); }

/* Toggle track checked */
.tgl input:checked + .tgl-track { background: var(--accent) !important; }

/* AccueilApp — override inline style sur nav */
.app-shell nav { display:none !important; }
`;
  document.head.appendChild(s);
})();

// ═══════════════════════════════════════════════════════════
// COMPOSANT : ChoixTheme (premier lancement)
// ═══════════════════════════════════════════════════════════
function ChoixTheme({ onChoix }){
  const[hovered, setHovered] = React.useState(null);
  const PREVIEWS = [
    // 1 Institutionnel
    function P1(){
      return CE('div',{style:{height:'100%',background:'#f0f4f8',position:'relative',display:'flex'}},
        CE('div',{style:{width:36,background:'linear-gradient(180deg,#1e3a8a,#197d89)',display:'flex',flexDirection:'column',alignItems:'center',padding:'8px 0',gap:5}},
          CE('div',{style:{width:22,height:22,borderRadius:6,background:'rgba(255,255,255,.2)',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center'}},'🖥️'),
          ...[1,2,3,4].map(i=>CE('div',{key:i,style:{width:22,height:16,borderRadius:4,background:i===1?'rgba(255,255,255,.25)':'rgba(255,255,255,.08)'}}))
        ),
        CE('div',{style:{flex:1,display:'flex',flexDirection:'column'}},
          CE('div',{style:{height:24,background:'#fff',borderBottom:'1px solid #e2e8f0',display:'flex',alignItems:'center',padding:'0 8px',gap:5}},
            CE('div',{style:{width:6,height:6,borderRadius:'50%',background:'#197d89'}}),
            CE('div',{style:{fontSize:8,fontWeight:700,color:'#1e3a8a'}}),'Historique'
          ),
          CE('div',{style:{flex:1,padding:6,display:'flex',flexDirection:'column',gap:4}},
            CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}},
              ...[['#1e3a8a','142'],['#197d89','83%']].map(([c,v],i)=>CE('div',{key:i,style:{background:'#fff',borderRadius:4,padding:'4px 6px',textAlign:'center',boxShadow:'0 1px 2px rgba(0,0,0,.06)'}},
                CE('div',{style:{fontSize:12,fontWeight:800,color:c}}),v
              ))
            ),
            ...[1,2,3].map(i=>CE('div',{key:i,style:{height:12,borderRadius:4,background:'#fff',boxShadow:'0 1px 2px rgba(0,0,0,.04)',borderLeft:'3px solid '+(i===1?'#1e3a8a':'#197d89')}}))
          )
        )
      );
    },
    // 2 Dark Teal
    function P2(){
      return CE('div',{style:{height:'100%',background:'#0d1117',position:'relative',display:'flex'}},
        CE('div',{style:{width:36,background:'#0d1117',borderRight:'1px solid #1e293b',display:'flex',flexDirection:'column',alignItems:'center',padding:'8px 0',gap:5}},
          CE('div',{style:{width:22,height:22,borderRadius:6,background:'linear-gradient(135deg,#0891b2,#0e7490)',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center'}},'🖥️'),
          ...[1,2,3,4].map(i=>CE('div',{key:i,style:{width:22,height:16,borderRadius:4,background:i===1?'#0891b2':'rgba(255,255,255,.05)'}}))
        ),
        CE('div',{style:{flex:1,display:'flex',flexDirection:'column'}},
          CE('div',{style:{height:24,background:'#0d1117',borderBottom:'1px solid #1e293b',display:'flex',alignItems:'center',padding:'0 8px',gap:5}},
            CE('div',{style:{width:6,height:6,borderRadius:'50%',background:'#22d3ee'}}),
            CE('div',{style:{fontSize:8,fontWeight:700,color:'#e2e8f0'}},'Historique')
          ),
          CE('div',{style:{flex:1,padding:6,display:'flex',flexDirection:'column',gap:4}},
            CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}},
              ...[['#22d3ee','142'],['#22d3ee','83%']].map(([c,v],i)=>CE('div',{key:i,style:{background:'#1e293b',borderRadius:4,padding:'4px 6px',textAlign:'center',border:'1px solid #2d3748'}},
                CE('div',{style:{fontSize:12,fontWeight:800,color:c}}),v
              ))
            ),
            ...[1,2,3].map(i=>CE('div',{key:i,style:{height:12,borderRadius:4,background:'#1e293b',border:'1px solid #2d3748',borderLeft:'3px solid #0891b2'}}))
          )
        )
      );
    },
    // 3 Sable
    function P3(){
      return CE('div',{style:{height:'100%',background:'#faf7f2',position:'relative',display:'flex'}},
        CE('div',{style:{width:36,background:'#2d2416',display:'flex',flexDirection:'column',alignItems:'center',padding:'8px 0',gap:5}},
          CE('div',{style:{width:22,height:22,borderRadius:6,background:'#c2410c',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center'}},'🖥️'),
          ...[1,2,3,4].map(i=>CE('div',{key:i,style:{width:22,height:16,borderRadius:4,background:i===1?'#c2410c':'rgba(255,255,255,.08)'}}))
        ),
        CE('div',{style:{flex:1,display:'flex',flexDirection:'column'}},
          CE('div',{style:{height:24,background:'#faf7f2',borderBottom:'1px solid #e7ddd0',display:'flex',alignItems:'center',padding:'0 8px',gap:5}},
            CE('div',{style:{width:6,height:6,borderRadius:'50%',background:'#c2410c'}}),
            CE('div',{style:{fontSize:8,fontWeight:700,color:'#2d2416'}},'Historique')
          ),
          CE('div',{style:{flex:1,padding:6,display:'flex',flexDirection:'column',gap:4}},
            CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}},
              ...[['#c2410c','142'],['#ea580c','83%']].map(([c,v],i)=>CE('div',{key:i,style:{background:'#fff',borderRadius:4,padding:'4px 6px',textAlign:'center',boxShadow:'0 1px 2px rgba(0,0,0,.06)',border:'1px solid #e7ddd0'}},
                CE('div',{style:{fontSize:12,fontWeight:800,color:c}}),v
              ))
            ),
            ...[1,2,3].map(i=>CE('div',{key:i,style:{height:12,borderRadius:4,background:'#fff',border:'1px solid #e7ddd0',borderLeft:'3px solid #c2410c'}}))
          )
        )
      );
    },
    // 4 Forêt
    function P4(){
      return CE('div',{style:{height:'100%',background:'#f0faf4',position:'relative',display:'flex'}},
        CE('div',{style:{width:36,background:'linear-gradient(180deg,#14532d,#166534)',display:'flex',flexDirection:'column',alignItems:'center',padding:'8px 0',gap:5}},
          CE('div',{style:{width:22,height:22,borderRadius:6,background:'rgba(255,255,255,.2)',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center'}},'🖥️'),
          ...[1,2,3,4].map(i=>CE('div',{key:i,style:{width:22,height:16,borderRadius:4,background:i===1?'rgba(255,255,255,.25)':'rgba(255,255,255,.08)'}}))
        ),
        CE('div',{style:{flex:1,display:'flex',flexDirection:'column'}},
          CE('div',{style:{height:24,background:'#fff',borderBottom:'1px solid #d1fae5',display:'flex',alignItems:'center',padding:'0 8px',gap:5}},
            CE('div',{style:{width:6,height:6,borderRadius:'50%',background:'#16a34a'}}),
            CE('div',{style:{fontSize:8,fontWeight:700,color:'#14532d'}},'Historique')
          ),
          CE('div',{style:{flex:1,padding:6,display:'flex',flexDirection:'column',gap:4}},
            CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}},
              ...[['#16a34a','142'],['#22c55e','83%']].map(([c,v],i)=>CE('div',{key:i,style:{background:'#fff',borderRadius:4,padding:'4px 6px',textAlign:'center',boxShadow:'0 1px 2px rgba(0,0,0,.06)'}},
                CE('div',{style:{fontSize:12,fontWeight:800,color:c}}),v
              ))
            ),
            ...[1,2,3].map(i=>CE('div',{key:i,style:{height:12,borderRadius:4,background:'#fff',boxShadow:'0 1px 2px rgba(0,0,0,.04)',borderLeft:'3px solid #16a34a'}}))
          )
        )
      );
    },
    // 5 Violet
    function P5(){
      return CE('div',{style:{height:'100%',background:'#1e1b2e',position:'relative',display:'flex'}},
        CE('div',{style:{width:36,background:'#13111f',borderRight:'1px solid #2d2b45',display:'flex',flexDirection:'column',alignItems:'center',padding:'8px 0',gap:5}},
          CE('div',{style:{width:22,height:22,borderRadius:6,background:'linear-gradient(135deg,#7c3aed,#9333ea)',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center'}},'🖥️'),
          ...[1,2,3,4].map(i=>CE('div',{key:i,style:{width:22,height:16,borderRadius:4,background:i===1?'#7c3aed':'rgba(255,255,255,.05)'}}))
        ),
        CE('div',{style:{flex:1,display:'flex',flexDirection:'column'}},
          CE('div',{style:{height:24,background:'#13111f',borderBottom:'1px solid #2d2b45',display:'flex',alignItems:'center',padding:'0 8px',gap:5}},
            CE('div',{style:{width:6,height:6,borderRadius:'50%',background:'#a78bfa'}}),
            CE('div',{style:{fontSize:8,fontWeight:700,color:'#e2e0ff'}},'Historique')
          ),
          CE('div',{style:{flex:1,padding:6,display:'flex',flexDirection:'column',gap:4}},
            CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}},
              ...[['#a78bfa','142'],['#a78bfa','83%']].map(([c,v],i)=>CE('div',{key:i,style:{background:'#2d2b45',borderRadius:4,padding:'4px 6px',textAlign:'center',border:'1px solid #3d3b60'}},
                CE('div',{style:{fontSize:12,fontWeight:800,color:c}}),v
              ))
            ),
            ...[1,2,3].map(i=>CE('div',{key:i,style:{height:12,borderRadius:4,background:'#2d2b45',border:'1px solid #3d3b60',borderLeft:'3px solid #7c3aed'}}))
          )
        )
      );
    }
  ];
  const PreviewComponents = PREVIEWS;
  return CE('div',{className:'theme-welcome-overlay'},
    CE('div',{style:{fontSize:36,marginBottom:8}},'🖥️'),
    CE('div',{className:'theme-welcome-title'},'Bienvenue — Ateliers CD47'),
    CE('div',{className:'theme-welcome-sub'},'Choisissez votre thème visuel — vous pourrez le changer à tout moment'),
    CE('div',{className:'theme-welcome-grid'},
      THEMES.map((t,i)=>CE('button',{
        key:t.id,
        className:'theme-welcome-card',
        onClick:()=>onChoix(t.id),
        onMouseEnter:()=>setHovered(t.id),
        onMouseLeave:()=>setHovered(null),
        style:{
          outline:hovered===t.id?'3px solid #38bdf8':'2px solid transparent',
          transform:hovered===t.id?'translateY(-4px)':'none',
          boxShadow:hovered===t.id?'0 16px 40px rgba(0,0,0,.5)':'0 2px 8px rgba(0,0,0,.3)',
          transition:'all .2s'
        }
      },
        CE('div',{className:'theme-welcome-preview'},
          CE(PreviewComponents[i],null)
        ),
        CE('div',{className:'theme-welcome-label'},t.icon+' '+t.name)
      ))
    ),
    CE('div',{className:'theme-welcome-footer'},'Votre choix est sauvegardé localement')
  );
}

// ═══════════════════════════════════════════════════════════
// COMPOSANT : ThemeSwitcher (dans la topbar)
// ═══════════════════════════════════════════════════════════
function ThemeSwitcher(){
  const[open,setOpen]=React.useState(false);
  const[current,setCurrent]=React.useState(getCurrentTheme()||'1');
  ...
  const curr=THEMES.find(t=>t.id===current)||THEMES[0];
  ...
  return CE('div',{ref,style:{position:'relative'}},
    ...
    open&&CE('div',{className:'theme-dropdown'},
      CE('div',{className:'theme-dropdown-title'},'Thème'),
      THEMES.map(t=>CE('button',{
        key:t.id,
        ...
        CE('span',{style:{
          ...
          background:t.swatch,boxShadow:'0 1px 4px rgba(0,0,0,.2)'
        }}),
        CE('span',null,t.icon+' '+t.name),
        t.id===current&&CE('span',{className:'theme-option-check'},'✓')
      ))
    )
  );
}
...
const PUBLICS_DEFAULT = [
  'Tous publics','Besoin particulier','Collèges','Jeunes',
  'Séniors','Insertion Pro','Interne','Autres'
];
...
// ── API v15.0 ──────────────────────────────────────────────
(function(){
  const MAX_RETRY=1;const RETRY_DELAY=3000;
  ...
  window.apiFetch=async function apiFetch(action,body={},_attempt=1){
    ...
    try{
      const res=await Promise.race([fetch(`${GS_URL}?${params.toString()}`),new Promise((_,r)=>setTimeout(()=>r(new Error('timeout')),TIMEOUT_MS))]);
      return res.json();
    }catch(err){
      if(err.message==='timeout'&&_attempt<=MAX_RETRY){console.warn(`[apiFetch] Timeout "${action}" — retry ${_attempt}/${MAX_RETRY}`);await new Promise(r=>setTimeout(r,RETRY_DELAY));return window.apiFetch(action,body,_attempt+1);}
      if(err.message==='timeout')throw new Error('Le serveur ne répond pas — réessaie dans quelques secondes.');
      throw err;
    }
  };
})();
...
async function loadCommunes47(){
  ...
  try{
    ...
    const data=await res.json();const result=[];
...
function ComboCommune({value,onChange,hasError}){
  ...
  return CE('div',{className:'combo-wrap',ref:wrapRef},
    ...
    apiLoading&&CE('div',{className:'combo-dropdown'},CE('div',{className:'combo-loading'},CE('span',{className:'spinner',style:{borderTopColor:'var(--accent)',borderColor:'var(--border-color)',marginRight:8}}),'Chargement des communes…')),
