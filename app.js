...
// ── Wrapper App avec choix thème ────────────────────────────
function AppWithTheme(){
  const[themeChosen,setThemeChosen]=React.useState(()=>!!getCurrentTheme());
  function handleChoixTheme(id){
    applyTheme(id);
    setThemeChosen(true);
  }
  if(!themeChosen){
    return CE(ChoixTheme,{onChoix:handleChoixTheme});
  }
  return CE(App,null);
}
...
function App(){
  ...
  if(view==='accueil'){
    ...
    return CE('div',null,
      CE('nav',{style:{background:'var(--nav-bg)'}},
      ...
      CE('div',{className:'app-topbar'},
        ...
        CE('div',{className:'app-topbar-right'},
          ...
          loading&&CE('span',{style:{fontSize:11,color:'#9ca3af',display:'flex',alignItems:'center',gap:4}},
            CE('span',{className:'spinner',style:{borderTopColor:'var(--accent)',borderColor:'var(--border-color)'}}), 'Chargement…'),
...
ReactDOM.createRoot(document.getElementById('root')).render(CE(AppWithTheme));